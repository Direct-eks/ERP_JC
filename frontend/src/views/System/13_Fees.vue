                            ...
                            <v-card outlined>
                                <v-card-text>
                                    <v-row class="my-2">
                                        ...
                                        <v-col cols="auto">
                                            <v-text-field v-model="newCategory.remark"
                                                          label="描述"
                                                          single-line
                                                          hide-details="auto"
                                                          outlined dense
...
export default {
    name: "Fees",
    created() {
        this.$store.watch(state => state.feeCategoryList, () => {
            const data = this.$store.state.feeCategoryList
            this.tableData = JSON.parse(JSON.stringify(data))
            this.treeData = this.$createTree(this.tableData, 'fee')
        })
    },
    beforeMount() {
    ...
    methods: {
        treeSelect(data) {
            ...
            this.tableData.forEach(item => {
                if (item.feeCategoryID === val.categoryID) {
                    this.currRow = [item]
                }
            })
        },
        addNewItem() {
            ...
            else if (nextLevelNumber < 36) {
                const endLevel = String.fromCharCode(String('a').charCodeAt(0) + nextLevelNumber - 10)
                newLevel = treeLevel + "-" + endLevel
                // console.log('new level: ', newLevel)
            }
            else {
                const endLevel = String.fromCharCode(String('A').charCodeAt(0) + nextLevelNumber - 36)
                newLevel = treeLevel + "-" + endLevel
                // console.log('new level: ', newLevel)
            }

            this.tableData.push({
                feeCategoryID: this.newItemIndex--,
                name: this.newCategory.name,
                remark: this.newCategory.remark,
                classification: this.newCategory.classification,
                treeLevel: newLevel
            })
        },
        previewChanges() {
        ...
        saveChanges() {
            this.$postRequest(this.$api.updateFeeCategories, {
                ...
                this.$store.commit('clearFeeCategories')
