...
public class AssemblyEntryController {
    ...
    private static final String ENTRY_TYPE1 = "拆入";
    private static final String ENTRY_TYPE2 = "拆出";
    ...
    public void createEntry(@RequestBody @Validated WarehouseEntryWithProductsVO entry) throws GlobalParamException {
        logger.info("PUT Request to /assemblyEntry/createEntry, data: {}", entry);
        ...
        assemblyEntryService.createEntry(entry, ENTRY_TYPE1, INBOUND1);
        assemblyEntryService.createEntry(entry, ENTRY_TYPE2, INBOUND2);
    }
    ...
    public List<WarehouseEntryWithProductsVO> getEntriesInDateRange(
        ...
        assemblyEntryService.getEntriesInDateRange(startDate, endDate, ENTRY_TYPE1, INBOUND1);
        return assemblyEntryService.getEntriesInDateRange(startDate, endDate, ENTRY_TYPE2, INBOUND2);
    }

    @ApiOperation(value = "", response = void.class)
    @PatchMapping("/modifyEntry")
    public void modifyEntry(@RequestBody @Validated WarehouseEntryWithProductsVO entry) {
        logger.info("PATCH Request to /assemblyEntry/modifyEntry, entry: {}", entry);

        assemblyEntryService.modifyEntry(entry, ENTRY_TYPE1, INBOUND1);
        assemblyEntryService.modifyEntry(entry, ENTRY_TYPE2, INBOUND2);
    }
